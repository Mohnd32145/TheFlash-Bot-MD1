let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  // اختيار رابط عشوائي من القائمة وإرسال الفيديو
  const randomVideo = global.pies[Math.floor(Math.random() * global.pies.length)];
  await conn.sendMessage(
    m.chat, 
    { video: { url: randomVideo }, caption: global.veeeee }, 
    { quoted: m }
  );
};

// تعريف أوامر المساعدة والوسوم
handler.help = ['فيديو قران'];
handler.tags = ['قران'];
handler.command = /^(فيد قران)$/i;
handler.limit = true;
